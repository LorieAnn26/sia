<?php 
    ...
    function getRecentOrders($limit = 10){
		...
		$stmt = $conn->prepare("SELECT * FROM sales WHERE sales.date_created ORDER BY sales.date_created DESC LIMIT $limit");
		$stmt->execute();
    ...
    function getSales($start, $end){
        ...
        $conn = $GLOBALS['conn_pos'];
		$stmt = $conn->prepare("SELECT * FROM sales WHERE sales.date_created >= '$start 00:00:00' and sales.date_created <= '$end 23:59:59' ");
		$stmt->execute();
		$sales = $stmt->fetchAll(PDO::FETCH_ASSOC);
